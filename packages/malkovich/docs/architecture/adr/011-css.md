...
```mermaid
graph TB
    subgraph "Common Package (MIT)"
        Theme[theme.css<br/>Single Entry Point]
        Typography[_typography.css]
        Variables[_variables.css]
        Tooltip[common/_tooltip.css]
    ...
    subgraph "Expressio App"
        ExpressioApp[app.css]
        ExpressioVars[_variables.css<br/>Project Overrides]
        ExpressioComponents[components/**/*.css]

        ExpressioApp -->|@import| Theme
        ExpressioApp --> ExpressioVars
        ExpressioApp --> ExpressioComponents
    end

    subgraph "Pyrite App"
        PyriteApp[app.css]
        PyriteVars[_variables.css<br/>Project Overrides]
        PyriteComponents[components/**/*.css]

        PyriteApp -->|@import| Theme
        PyriteApp --> PyriteVars
        PyriteApp --> PyriteComponents
    end

    style Theme fill:#fff4e1
    style ExpressioApp fill:#e1f5ff
    style PyriteApp fill:#e1f5ff
```

**Pattern**: Single `theme.css` import provides all shared styles. Projects extend with project-specific overrides.

### Design Token Hierarchy

```mermaid
graph LR
    Root[:root<br/>OKLCH Colors]
    Light[.theme-light<br/>Light Theme]
    Dark[.theme-dark<br/>Dark Theme]

    Root -->|defines| Tokens[Design Tokens<br/>--spacer-*, --font-*, --info-*]
    Light -->|uses| Tokens
    Dark -->|uses| Tokens

    Tokens -->|consumed by| Components["Components<br/>Use CSS variables"]
...
**Rule**: All colors use OKLCH for perceptual uniformity. Components reference tokens, never hardcoded values.
...
- [ADR-004: Preact WebSocket Architecture](./004-preact-ws.md)
